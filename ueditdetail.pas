...
begin
  if (DBEdtSubTotal.Text <> '')
    ...
    DBEdtTax.Text :=
      IntToStr(Round(StrToInt(DBEdtSubTotal.Text) / (1 + GetTaxRate) * GetTaxRate));
...
function TFrmEditDetail.GetTaxRate: Real;
begin
  Result := FTaxRate;
end;

procedure TFrmEditDetail.SetTaxRate(TaxRate: Real);
begin
  FTaxRate := TaxRate;
end;
